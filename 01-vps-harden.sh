...
# Check if we already hardened SSH (look for our marker in sshd_config)
if grep -q "generated by 01-vps-harden.sh" /etc/ssh/sshd_config 2>/dev/null; then
...
if [ "${RECONFIG_SSH:-y}" = "y" ]; then
  ...
  # Write hardened config directly to sshd_config (drop-in .d/ is unreliable across distros)
  cat > /etc/ssh/sshd_config << EOF
# VPS Hardened SSH Config — generated by 01-vps-harden.sh
# Original backed up to /etc/ssh/sshd_config.bak.*
...
EOF
  ...
  # Remove any old drop-in config to avoid conflicts
  rm -f /etc/ssh/sshd_config.d/99-hardened.conf 2>/dev/null

  # Validate config before restarting
  if ! sshd -t 2>/dev/null; then
    warn "SSH config test failed. Check /etc/ssh/sshd_config for errors."
    sshd -t
  fi
