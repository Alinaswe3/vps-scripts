...
# --- Default catch-all block — rejects direct IP access on port 80 and 443 ---
# Written once; skipped if already exists so re-runs don't overwrite it.
DEFAULT_CONF="/etc/nginx/sites-available/default-catch-all"
if [ ! -f "$DEFAULT_CONF" ]; then
  # Ensure snakeoil self-signed cert exists (required for SSL default_server)
  if [ ! -f /etc/ssl/certs/ssl-cert-snakeoil.pem ]; then
    apt-get install -y -q ssl-cert 2>/dev/null || true
  fi
  cat > "$DEFAULT_CONF" << 'DEFAULTEOF'
# Default catch-all — blocks direct IP access and unknown hostnames
# Generated by vps-nginx-config
...
server {
    ...
    # Self-signed cert required for SSL default_server — nginx won't start without one
...
DEFAULTEOF
  ln -sf "$DEFAULT_CONF" "/etc/nginx/sites-enabled/default-catch-all"
  echo -e "${GREEN}[OK]${NC} Default catch-all block created (direct IP access blocked)."
fi
